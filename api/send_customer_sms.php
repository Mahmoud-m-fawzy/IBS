...
$query = "SELECT name, phone FROM customers WHERE phone IS NOT NULL AND phone != ''";
...
$templateHeader = "📢 *رسالة من IBS* 📢\n" . str_repeat("━", 20) . "\n\n";
$templateFooter = "\n\n" . str_repeat("━", 20) . "\n*IBS Mobile Shop*";
$templatedMessage = $templateHeader . $message . $templateFooter;

if (empty($customers)) {
    echo json_encode(['success' => false, 'message' => 'No customers with phone numbers found']);
    exit;
}
...
// Log the broadcast
error_log("[WHATSAPP CUSTOMER BROADCAST] Result: " . $results['success'] . "/" . ($results['success'] + $results['failed']));
